...
def _rule_based_market_analysis(data: dict) -> dict:
    ...
    # ── Geo Risk: ידע בסיסי + סריקת כותרות ─────────────────────────────
    # קונפליקטים מתמשכים — תמיד רלוונטיים, ללא תלות בכותרות RSS
    _BASELINE_RISKS = [
        "מלחמת רוסיה-אוקראינה",
        "מתיחות ישראל/ארה\"ב מול איראן",
    ]
    _geo_keywords = {
    ...
    _found_risks = list(_BASELINE_RISKS)  # תמיד מתחיל עם הקונפליקטים הבסיסיים
    for _kw, _label in _geo_keywords.items():
        if _kw in _all_heads_lower and _label not in _found_risks:
            _found_risks.insert(0, _label)  # כותרות בפועל מקבלות עדיפות
            if len(_found_risks) >= 3:
                break
    _geo_risk = " | ".join(_found_risks[:2])

    # ── פירוט שוק ספציפי לפי הקונפליקטים שזוהו ────────────────────────
    _geo_details_pool = []
    _risks_lower = " ".join(_found_risks).lower()

    if any(k in _risks_lower for k in ("רוסיה", "אוקראינה", "russia", "ukraine")):
        _geo_details_pool.append(
            "מלחמת רוסיה-אוקראינה ממשיכה ללחוץ על מחירי האנרגיה באירופה ועל שרשרת האספקה של חיטה ודשנים — "
            "מה שמוסיף לאינפלציה הגלובלית. מניות ביטחון (Lockheed, RTX, Rheinmetall) נהנות מגידול בתקציבי הגנה של נאט\"ו. "
            "הנפט מושפע ישירות מהסנקציות על הייצוא הרוסי — עלייה במחיר הנפט תלחץ על הצרכן ותאט את הצמיחה."
        )
    if any(k in _risks_lower for k in ("איראן", "ישראל", "iran", "israel", "hezbollah", "hamas")):
        _geo_details_pool.append(
            "המתיחות בין ישראל/ארה\"ב לאיראן מציבה סיכון מיידי למיצר הורמוז — דרכו עובר כ-20% מסחר הנפט הגלובלי. "
            "סגירה חלקית עלולה לשגר את מחיר הנפט מעל $120 לחבית ולגרום לשוק עולמי ל-Risk-Off חריף. "
            "הזהב והדולר מתחזקים בתרחיש זה; מניות טכנולוגיה וצמיחה הן הפגיעות ביותר."
        )
    if any(k in _risks_lower for k in ("סין", "מכסים", "china", "tariff")):
        _geo_details_pool.append(
            "מלחמת הסחר סין-ארה\"ב פוגעת ישירות בשרשרת האספקה של מוליכים למחצה ובענף הטכנולוגיה. "
            "חברות כמו Apple, NVIDIA ו-TSMC חשופות לסיכון שיבוש ייצור. "
            "מניות תעשייה ויצרנים אמריקאים עשויים להרוויח מהעברת ייצור חזרה לארה\"ב (Reshoring)."
        )
    ...
    if _geo_details_pool:
        ...
        _geo_risk_detail = (
            "אי-ודאות גיאופוליטית מעלה ביקוש למקלטים בטוחים: זהב, אג\"ח ממשלתי ודולר. "
            "בסביבה כזו, סקטורים הגנתיים (שירותים, בריאות, אנרגיה) מציגים ביצועי יתר על מניות צמיחה. "
            "מומלץ להפחית חשיפה לאזורים הגיאוגרפיים הנמצאים בסכסוך ולהגדיל רכיב נזילות בתיק."
        )
